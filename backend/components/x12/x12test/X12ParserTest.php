...
use backend\components\x12\FormatException;
use backend\components\x12\X12Parser;
use backend\components\x12\Cf;
use Yii;

class X12ParserTest extends FormatException {

	const EXPECTED_X12_TOSTRING = 
	...
	private function loadCf() {
		$cfX12 = new Cf("X12");
		$cfISA = $cfX12->addChild("ISA", "ISA");
		$cfGS = $cfISA->addChild("GS", "GS");
		$cfST = $cfGS->addChild("ST", "ST", "835", 1);
		$cfST->addChild("1000A", "N1", "PR", 1);
		$cfST->addChild("1000B", "N1", "PE", 1);
		$cf2000 = $cfST->addChild("2000", "LX");
		$cf2100 = $cf2000->addChild("2100", "CLP");
	...
	public function testParseFile() {
		$parser = new X12Parser($this->loadCf());
		$f1 = Yii::$app->basePath.'/components/x12/x12test/example835One.txt';
		$x12 = $parser->parse($f1);
		// return self::EXPECTED_X12_TOSTRING === $x12->toString() ? "right" : "wrong";
		// return $x12->toXML() === self::EXPECTED_X12_TOXML ? "right" : "wrong";
		return 28 === $x12->size() ? "right" : "wrong";
	}

	// public function testParseInputStream() extends FormatException {
